  ...
  if (analogRead(Before) == 1023){
    switch(currentScreen){
      case 0: // Pace --> Units
        display.setPaceMin(totalPace / 60);
        display.setPaceSec(totalPace % 60);
//        calculateVelocity(); //todo: calc velocity
        display.renderScreenUnits();
        currentScreen = 4;
        break;
      case 1: // Length --> Pace
...
//  digitalWrite(debuggingPin,!digitalRead(debuggingPin));
  ...
  if((x<=0)&(forward==false)){
    forward = true;
    velocityX = -velocityX;
    //TODO: might need to move this out, not sure...
    lapCount++;
//    display.renderScreenSwim(lapCount);
  }
...
//  Serial.println((int)sweepSignal);
  ...
  if ((analogRead(startButton) == 1023) && (mode == 1)){ //if user pauses during swim
    mode = 2; //pause mode
...
//*************************************************************
void calculateVelocity(){    //UI-Galvo interface
  velocityX = 100.0/((double)totalPace);
}
//*************************************************************
